        ...
        public void AddDefaultUsers()
        {
            using (var context = new RestaurantContext())
            {
                ...
                foreach(var person in employees)
                {
                    ...
                    // u represents a given user where the
                    // user's WaiterID has a value
                    // AND the user's WaiterID matches this employee's WaiterID)
                    if (!Users.Any(u => u.WaiterID.HasValue && u.WaiterID.Value ==
                        person.WaiterID))
                    {
                        // Add them as a user...
                        string userName = string.Format(STR_USERNAME_FORMAT,
                        ...
                        // NOTE: The following line of code needs to use the this keyword
                        // in order to have access to teh  Create() extension method.
